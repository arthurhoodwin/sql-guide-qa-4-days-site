...
## 14) Формат ожидаемого вывода в задачах Дня 2

Для JOIN/GROUP BY задач обязательно фиксируй ожидаемый формат ответа до написания SQL:

1. Набор колонок.
2. Агрегаты и их алиасы (`total_paid`, `orders_cnt` и т.д.).
3. Сортировка результата.
4. Пример строки результата.

Примеры:

- «Пользователи без заказов»:
  - колонки: `user_id`, `username`;
  - пример: `(4, 'oleg')`.
- «Сумма оплаченных заказов по пользователям»:
  - колонки: `user_id`, `total_paid`;
  - пример: `(1, 7500)`;
  - сортировка: `total_paid DESC`.
- «Количество заказов по статусам»:
  - колонки: `status`, `cnt`;
  - пример: `('paid', 4)`.
- «Пользователи с количеством заказов > 2»:
  - колонки: `user_id`, `orders_cnt`;
  - пример: `(3, 3)`.
